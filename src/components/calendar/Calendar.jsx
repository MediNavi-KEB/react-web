    ...
    const renderCells = () => {
        ...
        while (day <= endDate) {
            for (let i = 0; i < 7; i++) {
                ...
                const dateKey = format(day, 'yyyy-MM-dd');
                const isSelected = isSameDay(day, selectedDate);
                const isHoliday = holidays.includes(dateKey) || day.getDay() === 0 || day.getDay() === 6;
                const isOutsideMonth = day.getMonth() !== currentMonth.getMonth(); // 해당 달이 아닌 날짜 확인
    
                days.push(
                    <div
                        className={`cal-col cal-cell ${isOutsideMonth ? 'cal-outside-month' : ''} ${isSelected ? 'cal-selected' : ''}`}
                        key={dateKey}
                        data-date={day}
                        onClick={() => handleDateClick(cloneDay)}
                        style={{ cursor: 'pointer' }}
